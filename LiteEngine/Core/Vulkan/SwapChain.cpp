...
void SwapChain::CreateSwapChain(const uint32_t width, const uint32_t height,
                                const vk::raii::SurfaceKHR& surface)
{
    auto sCapabilities = m_DeviceRef.GetPhysicalDevice().getSurfaceCapabilitiesKHR(surface);
    m_ImageFormat = ChooseSwapchainSurfaceFormat(
        m_DeviceRef.GetPhysicalDevice().getSurfaceFormatsKHR(surface));
    m_Extent = { std::clamp<uint32_t>(width, sCapabilities.minImageExtent.width,
                                      sCapabilities.maxImageExtent.width),
                 std::clamp<uint32_t>(height, sCapabilities.minImageExtent.height,
                                      sCapabilities.maxImageExtent.height) };
    uint32_t minImageCount = (prefImageCount > sCapabilities.maxImageCount)
        ? sCapabilities.maxImageCount
        : prefImageCount;
    ...
    swapCreateInfo.presentMode
        = ChooseSwapPresentMode(m_DeviceRef.GetPhysicalDevice().getSurfacePresentModesKHR(surface));
...
vk::Format
SwapChain::ChooseSwapchainSurfaceFormat(const std::vector<vk::SurfaceFormatKHR>& aFormats)
{
...
vk::PresentModeKHR
SwapChain::ChooseSwapPresentMode(const std::vector<vk::PresentModeKHR>& aPresentModes)
{
